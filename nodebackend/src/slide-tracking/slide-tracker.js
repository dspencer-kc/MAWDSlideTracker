...
function printSlides(request, response, callback) {
  ...
  // 8/11/22 -- pull dr initials and dept in seperate left joins to prevent blank data
  var strSQL = `  
  SELECT 
         tblSlides.AccessionID,
         tblSlides.PartDesignator,
         ...
         tblSlides.Status,
        tblCassetteColorHopperLookup.Color   AS SlideDistributionKeyword, 
        (select initials from copath_c_d_person cc where copath_p_stainprocess.orderedby_id = cc.id)      AS OrderPathInitials, 
        (select name from copath_c_d_department where copath_p_stainprocess.wkdept_id = copath_c_d_department.id )          AS StainDept
        FROM   tblSlides 
        LEFT JOIN copath_p_stainprocess ON tblSlides.BlockStainInstID = copath_p_stainprocess._blockstaininstid
        LEFT JOIN tblBlock ON tblSlides.BlockID = tblBlock.BlockID
        LEFT JOIN tblCassetteColorHopperLookup ON tblBlock.Hopper = tblCassetteColorHopperLookup.HopperID
        WHERE (((tblSlides.BlockID) = '${strBlockID}') AND tblSlides.ToBePrinted = TRUE);`;
  ...
  con.query(strSQL, function (err, result) {
    if (err) {
      ...
      Object.keys(result).forEach(function (key) {
        ...
        var strSlideFlatFileFullName =
          strSlideQueuePath + row.SlideID + "_" + fileDate + ".txt";

        fs.writeFile(
          strSlideFlatFileFullName,
          strFileWriteData,
...
/*qryCaseInquiry*/
      ...
      WHERE  (( ( tblSlides.AccessionID ) LIKE "` +
    strStrAccessionID +
    `"))
...
function GetBlockData(request, response, callback) {
  ...
  var strSQL =
    `SELECT * FROM OPENLIS.tblBlock WHERE \`BlockID\` = '` + blockID + `';`;
...
function SetBlockData(request, response, callback) {
  ...
  if (!TimesScannedAtEmbedding) {
    TimesScannedAtEmbedding = 1;
  } else {
    TimesScannedAtEmbedding = TimesScannedAtEmbedding + 1;
  }
  ...
  con2.query(strSQL, function (err, result) {
    console.log(strSQL);
    if (err) {
      console.log("err:", err);
      response.send("error");
